...
public class client{
    ...
    public void connect(){
        server s = new server();
        System.out.printf("\nCLIENT : Richiesta di apertura comunicazione con il server\n indirizzo IP (Server) = %s\t numero di porta (Server) = %s\n", s.getIndirizzoIP(), s.getPorta());
    }
    
    public int write(){
        String dato;
        Scanner t = new Scanner(System.in);
        System.out.println("SERVER : inserire l'operazione desiderata = ");//scrivere "quit" per uscire dalla comunicazione
        dato = t.nextLine();
